...
TRIAL_CONDITIONS = {
    ...
    26: { "type": 'wide_to_narrow', "segment1Width": 0.05, "segment2Width": 0.01, "curvature": 0.0, "label": "Wide-to-narrow, narrow=0.01" },
    27: { "type": 'wide_to_narrow', "segment1Width": 0.05, "segment2Width": 0.02, "curvature": 0.0, "label": "Wide-to-narrow, narrow=0.02" },
    28: { "type": 'wide_to_narrow', "segment1Width": 0.05, "segment2Width": 0.03, "curvature": 0.0, "label": "Wide-to-narrow, narrow=0.03" },
}
...
def _build_wide_to_narrow_config(segment1_width, segment2_width, curvature):
    """Create task config for a wide-to-narrow tunnel."""
    env_dict = {
        "env_type": "tunnel_steering_smooth",
        "screen_width": 460,
        "screen_height": 260,
        ...
        "curvature": curvature,
        "max_steps": 800,
        # Use segment2_width so the cursor reliably terminates at the tunnel end
...
def generate_centerline(trial_id):
    ...
    elif cond.get("type") == "wide_to_narrow":
        _, centerline = _build_wide_to_narrow_config(
            cond["segment1Width"], cond["segment2Width"], cond["curvature"]
        )
...
def run_simulator_for_trial(sim, trial_id, n_runs=1):
    ...
    width = cond.get("width", cond.get("segment1Width", 0.02))
    
    if tunnel_type == "sigmoidal":
        task_config, centerline = _build_sigmoidal_config(width, cond["curvature"])
    elif tunnel_type == "wide_to_narrow":
        task_config, centerline = _build_wide_to_narrow_config(
            cond["segment1Width"], cond["segment2Width"], cond["curvature"]
        )
    else:
        ...
        if args.include_baseline:
            ...
            if not args.per_participant:
                ...
                if bl_needs_run:
                    ...
                    baseline_sim = BaselineSimCls(str(config_path))
            ...
            for tid, m in trials.items():
                ...
                for k in metric_keys:
                    if k in m:
                        ...
                        pt[tid][k].append(m[k])
        avg = lambda d: {k: float(np.mean(v)) if v else None for k, v in d.items()}
        ta = {}
