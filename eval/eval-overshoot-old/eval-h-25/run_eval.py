    ...
    try:
        for run_id in range(n_runs):
            ...
            records.append({
                ...
                "terminated_by_dwell": ct < task_config.get("max_steps", 800) * interval,
            })
    finally:
        os.unlink(task_file)
...
def save_trial_results_summary(participant_id, trial_id, human_data, model_data,
    ...
    m_records_all = model_data.get("records", [])  # model_data is the unfiltered one
    ...
    if m_trajs and centerline:
        ...
        if all_lat_h and all_lat_m:
            ...
            APPROACH_START_20 = 80
            APPROACH_START_10 = 90
            approach_spd_corr_20 = speed_profile_correlation(mean_spd_m[APPROACH_START_20:], mean_spd_h[APPROACH_START_20:])
            approach_spd_rmse_20 = speed_profile_rmse(mean_spd_m[APPROACH_START_20:], mean_spd_h[APPROACH_START_20:])
            approach_spd_corr_10 = speed_profile_correlation(mean_spd_m[APPROACH_START_10:], mean_spd_h[APPROACH_START_10:])
            approach_spd_rmse_10 = speed_profile_rmse(mean_spd_m[APPROACH_START_10:], mean_spd_h[APPROACH_START_10:])
            ...
            agg = {
                ...
                "approach_speed_corr_20": float(approach_spd_corr_20),
                "approach_speed_rmse_20": float(approach_spd_rmse_20),
                "approach_speed_corr_10": float(approach_spd_corr_10),
                "approach_speed_rmse_10": float(approach_spd_rmse_10),
                "terminated_by_dwell_rate": float(sum(
                    1 for r in m_records_all if r.get("terminated_by_dwell", False)
                ) / len(m_records_all)) if m_records_all else None,
            }
...
def main():
    ...
    metric_keys = ["lateral_rmse_mean", "speed_rmse_mean", "speed_corr_mean", "time_diff_mean",
               ...
               "goal_approach_speed_diff", "approach_speed_corr_20", "approach_speed_rmse_20",
               "approach_speed_corr_10", "approach_speed_rmse_10",
               "terminated_by_dwell_rate"]
    ...
    ts_fields = ["trial_id", "label", "split", "tunnel_type", "width",
                ...
                "goal_approach_speed_diff", "approach_speed_corr_20", "approach_speed_rmse_20",
                "approach_speed_corr_10", "approach_speed_rmse_10",
                "terminated_by_dwell_rate","n_human", "n_model"]
    if has_baseline:
    ...
    with open(trial_summary_csv, 'w', newline='') as f:
        ...
        for tid in sorted(summary_by_trial.keys()):
            ...
            row = {
                ...
                "approach_speed_corr_20": round(ta["approach_speed_corr_20"], 4) if ta.get("approach_speed_corr_20") is not None else "",
                "approach_speed_rmse_20": round(ta["approach_speed_rmse_20"], 6) if ta.get("approach_speed_rmse_20") is not None else "",
                "approach_speed_corr_10": round(ta["approach_speed_corr_10"], 4) if ta.get("approach_speed_corr_10") is not None else "",
                "approach_speed_rmse_10": round(ta["approach_speed_rmse_10"], 6) if ta.get("approach_speed_rmse_10") is not None else "",
                "terminated_by_dwell_rate": round(ta["terminated_by_dwell_rate"], 4) if ta.get("terminated_by_dwell_rate") is not None else "",
                "n_human": len(h_recs),
                "n_model": len(m_recs),
            }
            if has_baseline:
        ...
        for pid in sorted(aggregate_metrics.keys()):
            ...
            for tid_str in sorted(p_metrics.keys(), key=int):
                ...
                row = {
                    ...
                    "approach_speed_corr_20": round(m["approach_speed_corr_20"], 4) if m.get("approach_speed_corr_20") is not None else "",
                    "approach_speed_rmse_20": round(m["approach_speed_rmse_20"], 6) if m.get("approach_speed_rmse_20") is not None else "",
                    "approach_speed_corr_10": round(m["approach_speed_corr_10"], 4) if m.get("approach_speed_corr_10") is not None else "",
                    "approach_speed_rmse_10": round(m["approach_speed_rmse_10"], 6) if m.get("approach_speed_rmse_10") is not None else "",
                    "terminated_by_dwell_rate": round(m["terminated_by_dwell_rate"], 4) if m.get("terminated_by_dwell_rate") is not None else "",
                }
                if has_baseline:
